...
locals {
  customer_s3_access_role_name = var.customer_role_name
...
# --------------------------------------------------------
# Confluent Cloud Integration Provider AWS for tableflow
# --------------------------------------------------------
...
resource "confluent_provider_integration" "main" {
  ...
  aws {
    # customer_role_arn = var.cc_aws_iam_role_arn
    customer_role_arn = local.customer_s3_access_role_arn
  }
  display_name = var.cc_provider_integration_name
}
...
module "s3_access_role" {
  ...
  customer_policy_name             = var.customer_policy_name
}
